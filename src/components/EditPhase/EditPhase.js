...
class EditPhase extends Component {
	...
	addHashtags = () => {
		...
		this.setState({
			post_text:
				this.state.post_text + " " + this.props.store.tempPhase.hashtag,
		});
	};
	saveHashtags = () => {
		console.log(
			"clicked save hashtags with:",
			this.state.hashtags,
			this.props.store.tempPhase.event_id
		);
		axios({
			method: "PUT",
			url: "/api/hashtag",
	...
	render() {
		...
		const includedPosts = this.props.store.post.filter(
			(post) => post.phase_id === this.props.store.tempPhase.id
		);
			...
			<div id='editEventDiv'>
				...
				<div id='eventPromotionDuration' className='rounded'>
					<div>
						...
						<textarea
							...
							onChange={(event) => this.handleChange(event, "post_text")}
				...
				<div id='eventHashtags' className='rounded'>
					...
					<textarea
						...
						onChange={(event) =>
							this.handleChange(event, "hashtags")
						}></textarea>
					...
					{phase.hashtag === undefined ? (
						...
						<p className='centered'>
							Hashtags Character Counter {phase.hashtag.length}
						</p>
					)}
					<br />
					<br />
