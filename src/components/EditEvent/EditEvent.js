...
class EditEvent extends Component {
	...
	render() {
		const includedPhases = this.props.store.phase.filter(
			(phase) => phase.event_id === this.props.store.temp.event_id
		);
		...
		return (
			<div id='editEventDiv'>
				...
				<h2 className='centered' id='phases'>
					Phases
				</h2>
				...
				<div className='centered'>
					...
					<select
						name='eventType'
						value={this.props.store.temp.type}
						id='eventType'
				...
				<div id='eventSocial' className='rounded'>
					...
					<h4 className='centered' id='eventInfo'>
						Event Info
					</h4>
					<br />
					{this.state.toggleEdit === false && (
						<>
							...
							<img
								...
								src={this.props.store.temp.event_image}
								width='170px'></img>
							<br />
					...
					{this.state.toggleEdit === true && (
						<>
							...
							<input
								className='centeredImage'
								onChange={(event) => this.handleChange(event, "event_image")}
								placeholder={this.props.store.temp.event_image}
							/>
							<br />
							...
							<input
								...
								placeholder={this.props.store.temp.name}
							/>
							<br />
							<h5 className='editHeader'>Acronym</h5>
							<input
								className='centeredImage'
								onChange={(event) => this.handleChange(event, "acronym")}
								placeholder={this.props.store.temp.acronym}
							/>
							<br />
							...
							<input
								...
								placeholder={this.props.store.temp.website}
							/>
							<br />
							<button className='centeredImage btn-all' onClick={this.saveEdit}>
								Save
							</button>
						</>
					)}
				...
				<div id='eventCollaborators' className='rounded'>
							...
							<tbody>
								{this.props.store.otherUsersReducer.map((otherUser) => (
									<tr key={otherUser.id}>
										...
										<td>
											<button onClick={() => this.addCollaborator(otherUser)}>
												Add
											</button>
											<button className='btn-all'>Remove</button>
										</td>
									</tr>
								))}
							</tbody>
					...
					<br />
					<button className='centeredImage btn-all'>Add Collaborator</button>
					<br />
					<br />
				</div>
